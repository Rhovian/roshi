...
/// treasury, base oracle, default subaccounts, fee settings, deposit capacity,
/// and the external investment enablement flag. RBAC authorities are
/// intentionally handled by explicit `Set*Authority` and transfer
/// instructions. Pause flags are intentionally handled by `SetPauseFlags`;
/// access mode and Merkle root by `SetVaultAccess`.
...
pub fn try_update_vault_config(
    ...
    vault.deposit_cap = args.deposit_cap;
