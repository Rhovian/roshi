...
/// atomically replaces the mutable settings (oracle, enabled, routed, deposit
/// cap). Immutable fields (vault, asset mint, decimals) and the derived
/// custody account are preserved; the replacement is validated on store.
pub fn try_update_asset(accounts: &[AccountInfo], args: UpdateAssetArgs) -> ProgramResult {
    update_writable_asset_as_admin(accounts, |asset| {
        ...
        asset.deposit_cap_atoms = args.deposit_cap_atoms;
        Ok(())
    })
}
