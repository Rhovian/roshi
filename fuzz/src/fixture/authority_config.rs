    ...
    pub fn action_set_swap_authority(&mut self) -> bool {
        ...
        if !self.load_vault().manage_paused().unwrap_or(true) {
            let ix = self.swap_base_ix(self.swap_authority_alt.pubkey(), false, 0);
            let rotated_ok = submit(&mut self.ctx, ix, &[&self.swap_authority_alt.clone()]);
            fuzz_assert!(rotated_ok, "rotated swap authority could not execute a swap");
